...
void print_tree_LDR(const TREE* tree)
{
	if (! *tree)
		return;
	
	print_tree_LDR(& (*tree)->left);

	putchar((*tree)->data);

	print_tree_LDR(& (*tree)->right);
}

void print_tree_LRD(const TREE* tree)
{
	if (! *tree)
		return;
	
	print_tree_LRD(& (*tree)->left);
	print_tree_LRD(& (*tree)->right);

	putchar((*tree)->data);
}

void print_tree_layer(const TREE* tree)
{
	if (! *tree)
		return;
